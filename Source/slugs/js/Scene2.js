  ...
  processCommand(input = [], newSegment=true) {
    ...
    switch (cmd[0]) {
      case ifWord: {
        ...
        if(cmd.length < 6 || !(wordsIfConditionRight.includes(cmd.at(-3))) || !(cmd.at(-2) == thenWord) || !(wordsAction.includes(cmd.at(-1))) || !(cmd.includes(ENTITY_TYPES[0])||cmd.includes(ENTITY_TYPES.at(-1))) || cmd.length != new Set(cmd).size) { 
          logInput(output);
          logError(ifError);
          return;
        }
